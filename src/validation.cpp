...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, TxValidationState& state,
                ...
                ValidationCache& validation_cache, const Consensus::Params& params)
                EXCLUSIVE_LOCKS_REQUIRED(cs_main, pool.cs)
{
    AssertLockHeld(cs_main);
    AssertLockHeld(pool.cs);
    ...
    return CheckInputScripts(tx, state, view, flags, /* cacheSigStore= */ true, /* cacheFullScriptStore= */ true, txdata, validation_cache, params);
}
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckInputScripts(tx, state, m_view, scriptVerifyFlags, true, false, ws.m_precomputed_txdata, GetValidationCache(), m_active_chainstate.m_chainman.GetConsensus())) {
        // SCRIPT_VERIFY_CLEANSTACK requires SCRIPT_VERIFY_WITNESS, so we
        // need to turn both off, and compare against just turning off CLEANSTACK
        // to see if the failure is specifically due to witness validation.
        TxValidationState state_dummy; // Want reported failures to be from first CheckInputScripts
        if (!tx.HasWitness() && CheckInputScripts(tx, state_dummy, m_view, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, false, ws.m_precomputed_txdata, GetValidationCache(), m_active_chainstate.m_chainman.GetConsensus()) &&
                !CheckInputScripts(tx, state_dummy, m_view, scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, false, ws.m_precomputed_txdata, GetValidationCache(), m_active_chainstate.m_chainman.GetConsensus())) {
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckInputsFromMempoolAndCache(tx, state, m_view, m_pool, currentBlockScriptVerifyFlags,
                                        ws.m_precomputed_txdata, m_active_chainstate.CoinsTip(), GetValidationCache(), m_active_chainstate.m_chainman.GetConsensus())) {
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Force block reward don't halve when halvings times more than 3.
    if (halvings >= 3)
        halvings = 3;
...
std::optional<std::pair<ScriptError, std::string>> CScriptCheck::operator()() {
    ...
    if (VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, m_tx_out.nValue, cacheStore, *m_signature_cache, *txdata).WithParams(m_params), &error)) {
        return std::nullopt;
    } else {
...
bool CheckInputScripts(const CTransaction& tx, TxValidationState& state,
                       ...
                       std::vector<CScriptCheck>* pvChecks)
{
    return CheckInputScripts(tx, state, inputs, flags, cacheSigStore, cacheFullScriptStore, txdata, validation_cache, Consensus::Params{}, pvChecks);
}
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!CheckBlockContextFree(block, state, params.GetConsensus(), !fJustCheck)) {
        if (state.GetResult() == BlockValidationResult::BLOCK_MUTATED) {
        ...
        LogError("%s: Consensus::CheckBlockContextFree: %s\n", __func__, state.ToString());
        return false;
    }
    ...
    // future.
    static constexpr int BIP34_IMPLIES_BIP30_LIMIT = 1;
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            TxValidationState tx_state;
            if (fScriptChecks && !CheckInputScripts(tx, tx_state, view, flags, fCacheResults, fCacheResults, txsdata[i], m_chainman.m_validation_cache, m_chainman.GetConsensus(), parallel_script_checks ? &vChecks : nullptr)) {
                // Any transaction validation failure in ConnectBlock is a block consensus failure
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
...
bool Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    if (m_mempool) AssertLockHeld(m_mempool->cs);
    if (m_chain.Height() <= 0) {
        const CBlock& g = m_chainman.GetParams().GenesisBlock();
        if (!g.vtx.empty()) {
            CCoinsViewCache view(&CoinsTip());
            const CTransaction& tx = *g.vtx[0];
            COutPoint out(tx.GetHash(), 0);
            if (!view.HaveCoin(out)) {
                AddCoins(view, tx, 0);
                view.Flush();
                LogPrintf("[UTOCoin] Genesis UTXO loaded.\n");
            }
        }
    }
...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev = nullptr, bool fCheckPOW = true)
{
    // Check proof of work matches claimed amount
    if (fCheckPOW) {
        const bool is_genesis = block.hashPrevBlock.IsNull();
        if (!is_genesis && pindexPrev == nullptr) {
            return true;
        }

        const int block_height = is_genesis ? 0 : pindexPrev->nHeight + 1;
        const std::optional<uint256> pow_hash{GetBlockProofHash(block, block_height, pindexPrev, consensusParams)};
        if (!pow_hash) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "high-hash", "proof of work ancestry missing");
        }
...
bool CheckBlockContextFree(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckMerkleRoot)
{
    ...
    for (const auto& tx : block.vtx) {
        ...
        if (!CheckTransaction(*tx, tx_state)) {
            // CheckBlockContextFree() does context-free validation checks. The only
...
bool HasValidProofOfWork(const std::vector<CBlockHeader>& headers, const Consensus::Params& consensusParams, const CBlockIndex* first_prev)
{
    if (headers.empty() || first_prev == nullptr) {
        return true;
    }

    const int first_height = first_prev->nHeight + 1;
    for (size_t i = 0; i < headers.size(); ++i) {
        const int block_height = first_height + static_cast<int>(i);
        const int key_height = GetRandomXKeyBlockHeight(block_height, consensusParams);

        uint256 key;
        if (block_height < static_cast<int>(consensusParams.randomx_lag)) {
            key = GetRandomXKey(nullptr, block_height, consensusParams);
        } else if (key_height == 0) {
            key = consensusParams.hashGenesisBlock;
        } else if (key_height <= first_prev->nHeight) {
            const CBlockIndex* key_block = first_prev->GetAncestor(key_height);
            if (key_block == nullptr) {
                return false;
            }
            key = key_block->GetBlockHash();
        } else {
            const size_t header_offset = static_cast<size_t>(key_height - first_height);
            if (header_offset >= i) {
                return false;
            }
            key = headers[header_offset].GetHash();
        }

        if (!CheckProofOfWork(GetRandomXPoWHash(headers[i], key), headers[i].nBits, consensusParams)) {
            return false;
        }
    }
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        if (!CheckBlockHeader(block, state, GetConsensus(), pindexPrev)) {
...
bool ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    if (!CheckBlockContextFree(block, state, params.GetConsensus()) ||
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // CheckBlockContextFree() does not support multi-threaded block validation because CBlock::fChecked can cause data race.
        // Therefore, the following critical section must include the CheckBlockContextFree() call as well.
        LOCK(cs_main);

        // Skipping AcceptBlock() for CheckBlockContextFree() failures means that we will never mark a block as invalid if
        // CheckBlockContextFree() fails.  This is protective against consensus failure if there are any unknown forms of block
        // malleability that cause CheckBlockContextFree() to fail; see e.g. CVE-2012-2459 and
        // https://lists.linuxfoundation.org/pipermail/bitcoin-dev/2019-February/016697.html.  Because CheckBlockContextFree() is
        // not very expensive, the anti-DoS benefits of caching failure (of a definitely-invalid block) are not substantial.
        bool ret = CheckBlockContextFree(*block, state, GetConsensus());
        if (ret) {
            // Store to disk
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    // NOTE: Header PoW is checked by ContextualCheckBlockHeader().
    if (!ContextualCheckBlockHeader(block, state, chainstate.m_blockman, chainstate.m_chainman, pindexPrev)) {
        LogError("%s: Consensus::ContextualCheckBlockHeader: %s\n", __func__, state.ToString());
        return false;
    }
    if (!CheckBlockContextFree(block, state, chainparams.GetConsensus(), fCheckMerkleRoot)) {
        LogError("%s: Consensus::CheckBlockContextFree: %s\n", __func__, state.ToString());
        return false;
    }
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (nCheckLevel >= 1) {
            const std::optional<uint256> pow_hash{GetBlockProofHash(block, pindex->nHeight, pindex->pprev, consensus_params)};
            if (!pow_hash) {
                LogPrintf("Verification error: missing proof-of-work ancestry at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                return VerifyDBResult::CORRUPTED_BLOCK_DB;
            }
            if (!CheckProofOfWork(*pow_hash, pindex->nBits, consensus_params)) {
                LogPrintf("Verification error: found bad proof of work at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                return VerifyDBResult::CORRUPTED_BLOCK_DB;
            }
            if (!CheckBlockContextFree(block, state, consensus_params)) {
...
bool Chainstate::LoadGenesisBlock()
{
    ...
    try {
        ...
        const std::optional<uint256> pow_hash{GetBlockProofHash(block, 0, nullptr, params.GetConsensus())};
        if (!pow_hash) {
            LogError("%s: genesis block proof of work ancestry missing\n", __func__);
            return false;
        }
        if (!CheckProofOfWork(*pow_hash, block.nBits, params.GetConsensus())) {
            LogError("%s: genesis block proof of work failed\n", __func__);
            return false;
        }
