...
public:
    ...
    void do_working(uint32_t thread_idx) {
        ...
        for ( uint32_t i = begin;i < end; block.nNonce = i++ ) {
            ...
            // if (r % 10000 == 0) {
            //     sleep(r / 10000 % 10);
            if (CheckProofOfWork(GetRandomXPoWHash(block, m_powHashKey), block.nBits, params)) {
                // found
    ...
    uint32_t Mining(std::shared_ptr<CBlock> block, const uint256& pow_hash_key)
    {
        std::lock_guard<std::mutex> lk(mining_lock);

        m_pSharedBlock = block;
        m_powHashKey = pow_hash_key;
        m_bFound = false;
