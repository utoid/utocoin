...
FUZZ_TARGET(utxo_total_supply)
{
    ...
    if (Params().GetConsensus().randomx_epoch != 0) {
        if (outputWarning) {
            std::cout << "Warning: The utxo_total_supply test case has been skipped because it uses the RandomX proof-of-work algorithm, which is too expensive for this fuzz target." << std::endl;
            outputWarning = false;
        }
        return;
    }
